...
Module Semilattice.
  ...
  Class Semilattice (carrier : Type) :=
    { eq : carrier -> carrier -> Prop where "x ≡ y" := (eq x y) : sl_scope;
      eq_equiv :: Equivalence eq;
      add : Z -> carrier -> carrier;
      join : carrier -> carrier -> carrier;
      add_distr n m x : add n (add m x) ≡ add (n + m) x;
      ...
      succ_inj : forall n x y, add n x ≡ add n y -> x ≡ y;
      succ_join : forall n x y, add n (join x y) ≡ join (add n x) (add n y);
    }.

  Notation "x ≡ y" := (eq x y) (at level 90) : sl_scope.

  Definition le {A} {SL : Semilattice A} (x y : A) := join x y ≡ y.

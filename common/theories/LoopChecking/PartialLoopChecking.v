...
Definition to_clauses (prems : premises) (concl : premises) : clauses :=
  LevelExprSet.fold (fun lk cls => Clauses.add (prems, lk) cls) concl Clauses.empty.

Definition is_loop (cls : clauses) (t : premises) :=
...
Lemma univ_non_empty (u : premises) : ~ LevelSet.Empty (levels u).
...
Lemma nleq k k' : ~ k ≤ Some k' -> exists z, k = Some z /\ k' < z.
Proof.
  destruct k.

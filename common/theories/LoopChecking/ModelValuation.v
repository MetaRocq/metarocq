  ...
  Section Semantics.
    ...
    Section Interpretation.
      ...
      Definition clause_sem (cl : clause) : Prop :=
        ...
        le (interp_expr concl) (interp_nes prems).
  ...
  Proof.
    ...
    - intros v clls. red.
      ...
      have hge := interp_nes_ge (SL := Zsemilattice) v prems _ H.
      cbn in *. by lia.
    - move=> V Hcls.
  ...
  Lemma clauses_sem_entails_all {cls prems concl} :
    cls ⊢a prems → concl ->
    (forall V, clauses_sem V cls -> interp_nes V concl ≤ interp_nes V prems).
  Proof.
    intros ha V hcls.
    red in ha.
    move: ha.
    revert concl.
    refine (@interp_nes_elim _ _ (fun concl z => _ -> z ≤ interp_nes V prems) V _ _ _).
    - move=> le //=. move/(_ le).

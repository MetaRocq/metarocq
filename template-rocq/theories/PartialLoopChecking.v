...
Definition valid_clause (m : model) (cl : clause) :=
  ...
  | Some k0 =>
    let (l, k) := concl cl in
    level_value_above m l (Z.of_nat k + k0)
  end.
...
Definition update_value (wm : LevelSet.t × model) (cl : clause) : update_result :=
  ...
  | Some k0 =>
    ...
    (* Does the conclusion also hold?
        ...
        pass computes this.
      *)
    if level_value_above m l (Z.of_nat k + k0) then Holds
    else DoesntHold (LevelSet.add l w, update_model m l (Z.of_nat k + k0))
  end.
...
Definition check_model (cls : clauses) (m : model) : option (LevelSet.t × model) :=
  let '(modified, (w, m)) := check_model_aux cls (LevelSet.empty, m) in
  match LevelSet.is_empty w with
  | true => None
  | false => Some (w, m)
  end.

Lemma check_model_aux_subset {cls w v} :
  forall modified w' v', check_model_aux cls (w, v) = (modified, (w', v')) -> LevelSet.Subset w w'.
Proof.
  intros modified w' v'.
  unfold check_model, check_model_aux, check_clause_model. revert modified w' v'.
  eapply ClausesProp.fold_rec => //.
  { intros. noconf H0. reflexivity. }
  intros x a s' s'' hin nin hadd IH.
  intros modified' w' v'. destruct a.
  unfold update_value.
  destruct x as [prem [l k]]; cbn.
  destruct p.
  destruct min_premise as [k0|] eqn:hk0.
  2:apply IH.
  destruct level_value_above.
...
Proof.
  ...
  destruct check_model_aux eqn:cm. destruct p.
  eapply check_model_aux_subset in cm.
  destruct LevelSet.is_empty eqn:hem => //.
  eapply levelset_not_Empty_is_empty in hem. now intros [= <- <-].
Qed.
...
Definition value_le : relation (option Z) := opt_le Z.le.

Infix "≤" := value_le (at level 50).
...
Lemma check_model_aux_false {cls w acc w' acc'} : check_model_aux cls (w, acc) = (false, (w', acc')) -> w = w' /\ acc = acc'.
Proof.
  unfold check_model_aux, check_clause_model.
  revert w' acc'.
  eapply ClausesProp.fold_rec.
  - intros s emp w' acc' [=] => //.
  - intros cl [w'' m''] cls' cls'' incl nincls' incls''.
    intros IH w' acc'.
    destruct update_value eqn:upd => //.
    * intros [= -> ->]. now specialize (IH _ _ eq_refl).
    * intros [= -> ->]. now specialize (IH _ _ eq_refl).
Qed.
...
Proof.
  ...
  - intros cl [modified [w' m']] cls' cls'' incl nincls' incls''.
    ...
    * move: (@update_value_valid w' m' cl).
      destruct update_value eqn:upd => //. 1-2:intros vcl [= -> <-];
...
Proof. intros x l k map. exists k; split => //. reflexivity. Qed.
...
Proof.
  ...
  * intros [= <- <-].
    ...
    destruct min_premise as [k0|] eqn:hmin => //.
    destruct level_value_above eqn:hval => //.
    intros [= <-]. cbn.
    now eapply update_model_not_above.
Qed.
...
Proof.
  ...
  * intros [= <-].
    ...
    destruct cl as [prem [l k]] => /=.
    unfold level_value_above.
    destruct level_value as [val|] eqn:hval => //.
    case: Z.leb_spec => //.
Qed.
...
Definition strict_update m '(prems, (l, k)) m' :=
  exists v, min_premise m prems = Some v
  /\ ~~ level_value_above m l (Z.of_nat k + v) /\ level_value_above m' l (Z.of_nat k + v).

Definition has_strict_update cls m l m' := exists prems k, Clauses.In (prems, (l, k)) cls /\ strict_update m (prems, (l, k)) m'.

Definition has_strict_update_W cls m W m' :=
  (forall l, LevelSet.In l W -> has_strict_update cls m l m').

(** The values of levels in W result from consequences of existing constraints *)
Definition is_update cls m W m' :=
  has_strict_update_W cls m W m' /\ model_map_outside W m m'.
...
Lemma is_update_check_model_invariants cls m W m' : ~ LevelSet.Empty W -> is_update cls m W m' -> check_model_invariants cls LevelSet.empty m W m' true.
Proof.
  unfold is_update. intros hw [hupd hnupd].
  unfold check_model_invariants.
  eapply nEmpty_exists in hw as [l hl].
  split.
  - lsets.
  - intros hl' hin'.
    specialize (hupd _ hin') as [prems [k [v [clin [minp [nabove above]]]]]].
    rewrite LevelSet.union_spec. right.
    rewrite clauses_conclusions_spec. eexists; split; eauto.
  - specialize (hupd l hl) as [prems [k [v [clin [minp [nabove above]]]]]].
    eexists; split; tea. rewrite /valid_clause. rewrite minp //=.
    cbn. move: nabove; unfold level_value_above.
    destruct level_value eqn:hlev; try constructor.
    move: above. unfold level_value_above.
    destruct (level_value m' l) => //.
    move/Z.leb_le => hle. constructor.
    rewrite -Z.ltb_antisym in nabove. move/Z.ltb_lt: nabove. lia.
  - split.
    { intros l' k hm.
      destruct (inLevelSet W l').
      specialize (hupd _ H) as [prems [k' [v' [clin [minp [nabove above]]]]]].
      eapply negbTE in nabove.
      eapply level_value_not_above_spec in nabove.
      rewrite (level_value_MapsTo hm) in nabove.
      eapply level_value_above_MapsTo in above as [k'0 [mk'0 le']].
      exists k'0. split => //. depelim le'. depelim nabove; constructor; lia.
      specialize (hnupd _ H). exists k. split; [|reflexivity].
      now rewrite -hnupd. }
    red.
    intros l'. apply todo.
    exact hnupd.
  - intros l' hl'. specialize (hupd _ hl') as [prems [k' [v' [clin [minp [nabove above]]]]]].
    eapply level_value_above_MapsTo in above as [k2 [mk' leqk']]. depelim leqk'. now exists y.
Qed.
...
Lemma update_value_spec w m cl w' m' :
  update_value (w, m) cl = DoesntHold (w', m') ->
  LevelSet.Equal w' (LevelSet.add (clause_conclusion cl) w) /\ strict_update m cl m'.
Proof.
  unfold update_value.
  destruct min_premise eqn:hmin => //.
  destruct cl as [prems [concl k]]; cbn.
  destruct level_value_above eqn:habove => //.
  intros [= <-].
  intuition auto. reflexivity.
  rewrite hmin. exists z; split => //.
  rewrite habove. split => //.
  subst m'. unfold level_value_above.
  rewrite level_value_update_model. eapply Z.leb_le. lia.
Qed.
...
Proof.
  ...
  destruct min_premise.
  destruct concl => //.
  replace (level_value_above m t1 (Z.of_nat n + z)) with (level_value_above m0 t1 (Z.of_nat n + z)).
  2:now rewrite H0.
  destruct level_value_above => //.
  red. cbn. split => //.
...
Instance has_strict_update_proper : Proper (Clauses.Equal ==> LevelMap.Equal ==> eq ==> LevelMap.Equal ==> iff) has_strict_update.
Proof.
  intros ? ? eqcl ? ? eqm ? ? eqs ? ? eqm'.
  unfold has_strict_update, strict_update. subst y1.
  setoid_rewrite eqcl. setoid_rewrite eqm. now setoid_rewrite eqm'.
Qed.

Instance has_strict_update_W_proper : Proper (Clauses.Equal ==> LevelMap.Equal ==> LevelSet.Equal ==> LevelMap.Equal ==> iff) has_strict_update_W.
Proof.
  intros ? ? eqcl ? ? eqm ? ? eqs ? ? eqm'.
  unfold has_strict_update_W.
  now setoid_rewrite eqcl; setoid_rewrite eqm; setoid_rewrite eqs; setoid_rewrite eqm'.
Qed.
...
Proof.
  ...
  destruct (update_value (w', m') cl) eqn:upd.
  * intros [= <- <-]. subst.
    destruct modified. 2:{ cbn in inv |- *. intuition. }
    destruct inv.
    split => //.
  ...
  * intros [= <- ->].
    move: upd.
    unfold update_value.
    destruct min_premise as [hmin|] eqn:eqmin => //.
    destruct cl as [prem [l k]] => /=.
    destruct level_value_above eqn:hval => //.
    intros [= <- <-].
    ...
    { destruct inv.
      ...
      + rewrite clauses_conclusions_add.
        ...
        intuition eauto. cbn. apply H0 in H5. lsets.
      + setoid_rewrite Clauses.add_spec; setoid_rewrite LevelSet.add_spec.
        destruct H1 as [cl []]; exists cl; split => //. eauto. eauto.
        destruct (level_value m (concl cl)) as [vconcl|] eqn:hconcl; [|constructor].
        eapply opt_lt_le_trans; tea.
      ...
      + now eapply total_model_of_update. }
    { destruct inv as [inv htot]; noconf inv. split => //.
      + lsets.
      + rewrite clauses_conclusions_add.
        intros x. rewrite LevelSet.add_spec !LevelSet.union_spec LevelSet.singleton_spec.
        intuition eauto.
      + setoid_rewrite Clauses.add_spec; setoid_rewrite LevelSet.add_spec.
        exists (prem, (l, k)).
        split; tea; eauto.
        - unfold valid_clause. cbn. now rewrite eqmin hval /=.
        - cbn. rewrite level_value_update_model. now apply level_value_not_above_spec.
      + split.
        { now eapply update_model_not_above. }
        { eapply update_model_same_domain.
          eapply declcl. }
...
Proof.
  destruct cl as [prems [concl k]]. unfold strict_update.
  intros [minp [eqminp [ha hna]]] leq.
  exists minp. split => //. split => //.
  eapply level_value_above_mon; tea.
Qed.
...
(* Lemma strict_update_ext_left m cl m' m'' : strict_update m cl  m ⩽ m' -> strict_update m' cl m'' -> strict_update m cl m''.
Proof.
  destruct cl as [prems [concl k]]. unfold strict_update.
  intros leq [minp [eqminp [ha hna]]].
  exists minp. split => //. split => //.
  eapply level_value_above_mon; tea.
Qed.
 *)

Lemma strict_update_outside w m m' m'' cl :
  m ⩽ m' -> m' ⩽ m'' ->
  model_map_outside w m m' ->
  enabled_clause m cl ->
  ~ LevelSet.In (clause_conclusion cl) w -> strict_update m' cl m'' -> strict_update m cl m''.
Proof.
  intros mext mext' wout enabled nout.
  apply wout in nout.
  destruct cl as [prems [concl k]].
  unfold strict_update.
  intros [v [minv []]].
  cbn in nout.
  unfold enabled_clause in enabled. cbn in enabled. destruct min_premise eqn:hmin => //. exists z; split=> //.
  split => //.
  have hp := (min_premise_pres prems mext). rewrite hmin minv in hp. depelim hp.
  2:{ eapply level_value_above_mon; tea. eapply level_value_above_MapsTo'; tea. rewrite -nout. }
  Admitted.

Definition has_strict_update_from cls m l m' :=
  exists prems k, Clauses.In (prems, (l, k)) cls /\
    exists minter, m ⩽ minter /\ strict_update minter (prems, (l, k)) m'.

Definition has_strict_update_from_W cls m W m' :=
  forall l, LevelSet.In l W -> has_strict_update_from cls m l m'.

Lemma has_strict_update_W_trans cls m w m' cl m'' :
  m' ⩽ m'' ->
  has_strict_update_from_W cls m w m' ->
  model_map_outside w m m' ->
  strict_update m' cl m'' ->
  has_strict_update_from_W (Clauses.add cl cls) m (LevelSet.add (clause_conclusion cl) w) m''.
Proof.
  intros mleq updW outw su.
  destruct cl as [prems [concl k]].
  intros l; rewrite LevelSet.add_spec.
  destruct (inLevelSet w concl).
  - intros [->|inw].
    * cbn.
      apply updW in H.
      destruct H as [prems' [k' [hin hstr]]].
      unfold has_strict_update_from. setoid_rewrite Clauses.add_spec.
      do 2 eexists; split. right. exact hin.
      destruct hstr as [minter [leqinter hstr]].
      exists m'. split.
    Admitted.
      { transitivity minter => //. red. }
      move: su hstr. unfold strict_update.
      intros [v [minv []]] [minter [leqinter [v' [minv' []]]]].
      exists m'. split.
      { transitivity minter => //.
      {}
      exists v'. split => //. split => //. eapply level_value_above_mon; tea.
    * eapply updW in inw.
      destruct inw as [prems' [k' [hin hstr]]].
      red. do 2 eexists; split. rewrite Clauses.add_spec. right; tea.
      move: su hstr. unfold strict_update.
      intros [v [minv []]] [v' [minv' []]].
      exists v'. split => //. split => //. eapply level_value_above_mon; tea.
  - intros [->|inw].
    * unfold has_strict_update. cbn -[strict_update].
      exists prems, k. split. rewrite Clauses.add_spec. now left.


      have minp := min_premise_pres prems' mleq.
      rewrite minv' in minp. depelim minp.


      exists v. split => //. 2:split => //. *)
Admitted.

Lemma check_clause_model_update {cl cls w m w' m' m'' modified modified'} :
  is_update cls m w m' ->
  declared_model_level m (clause_conclusion cl) ->
  check_clause_model cl (modified, (w, m')) = (modified', (w', m'')) ->
  is_update (Clauses.add cl cls) m w' m''.
Proof.
  intros isupd hdecl.
  unfold check_clause_model.
  destruct update_value eqn:upd.
  * intros [= <- <-]. subst m''.
    red. unfold has_strict_update_W, has_strict_update. setoid_rewrite Clauses.add_spec.
    destruct isupd. intuition eauto.
    specialize (H _ H1) as [? [? [? ?]]]; intuition auto. do 3 eexists; intuition eauto.
  * intros [= <- <-]. subst m''.
    red.  unfold has_strict_update_W, has_strict_update. setoid_rewrite Clauses.add_spec.
    destruct isupd. intuition eauto.
    specialize (H _ H1) as [? [? [? ?]]]; intuition auto. do 3 eexists; intuition eauto.
  * intros [= <- ->].
    red.
    eapply update_value_spec in upd as [hw' hsupd].
    destruct isupd as [hupd hnupd]. setoid_rewrite hw'. clear w' hw'.
    destruct cl as [prems [concl k]]; cbn.
    split.
    { cbn. intros l [hl|hl]. subst l.
      do 2 eexists. repeat split; eauto. cbn -[strict_update].
      specialize (hupd l )
      move: upd; unfold update_value.
      destruct min_premise eqn:hmin.
      have sub := update_value pose proof (inLe)
...
Proof.
  ...
  revert modified w' m'.
  eapply ClausesProp.fold_rec.
  - intros s' e modified w' m' mof tot [= <- <- <-].
    split => //.
  - intros x ? s' s'' inx nins' hadd ih modified w' m' mof tot.
    ...
    specialize (ih _ _ _ ms' tot eq_refl).
...
Proof.
  intros mof tot.
  move/(check_model_aux_spec mof tot) => [] _ _ [cl [incl inval]] _ _ _ tot'.
  unfold is_model.
...
Proof.
  intros mof tot; unfold check_model, is_model.
...
Section InnerLoop.
  ...
  Lemma total_model_of_ext {W W' m m'} :
    total_model_of W m -> model_extension W' m m' -> total_model_of W m'.
  Proof.
    intros mof [ext _ _].
  ...
  Proof.
    intros []; cbn => htot. eapply total_model_of_ext; tea.
  Qed.
  ...
  Section innerloop_partition.
    ...
    #[tactic="idtac"]
    Equations? inner_loop_partition (m : model) (mW : total_model_of W m) : result W U cls m
    ...
    Proof.
      ...
      - apply mr'.
      - apply mr'.
      - eapply (check_model_spec_diff mr) in eqm as [eqw hm hext] => //. 2:apply mr.
      ...
      - rewrite check_model_is_model in eqm.
        1:{ eapply model_of_diff. apply mr. }
        apply mr.
...
Lemma valid_model_of {V W W' m cls} (m' : valid_model W W' m cls) :
...
Proof.
  ...
  - eapply (check_model_spec_V mof clsV) in eqm as [UW WU hcl ext] => //.
    ...
    split => //. lsets.
    exact (model_of_ext (valid_model_of mwc (model_of_ext mof ext)) H4) => //.
    eapply total_model_of_subset; [|apply hsub'].
    eapply valid_model_total; tea.
  - right.
    eapply (check_model_spec_V mof clsV) in eqm as [UW WU hcl ext] => //.
    eapply check_model_spec in eqm' as [].
    ...
    { split => //.
      ...
      now apply H7, WcW, wclswc. }
    eapply (strict_subset_leq_right _ (LevelSet.diff V Wc)).
    2:{ clear -UWc WcW UW WU H3 H4. lsets. }
    apply strict_subset_diff_incl => //. clear -H1 H6; lsets.
    eapply total_model_of_subset; [|apply hsub']. eapply valid_model_total; tea.
  - eapply mcls'.
  - apply mcls'.
  - auto.
  - exact mcls'.
  ...
  - eapply (check_model_spec_V mof clsV) in eqm as [UW WU hcl ext] => //.
    ...
    2:{ eapply model_of_subset. 2:exact clsV.
      exact (valid_model_of mwc (model_of_ext mof ext)). }
    split. 1-2:clear -hsub' hsub'' UV H1; lsets.
    eapply total_model_of_subset; [|apply hsub']; apply valid_model_total; tea.
  - eapply (check_model_spec_V mof clsV) in eqm as [UW WU hcl ext] => //.
  ...
  - rewrite check_model_is_model // in eqm'.
    ...
    refine (model_of_ext mof ext). auto. apply mwc.
  - eapply (check_model_spec_V mof clsV) in eqm as [UW WU hcl ext] => //.
...
Definition zero_model levels :=
  LevelSet.fold (fun l acc => LevelMap.add l (Some 0%Z) acc) levels (LevelMap.empty _).

Definition add_max l k m :=
  match LevelMap.find l m with
  | Some (Some k') =>
    if (k' <? k)%Z then LevelMap.add l (Some k) m
    else m
  | _ => LevelMap.add l (Some k) m
  end.
Lemma In_add_max l l' k acc :
  LevelMap.In (elt:=option Z) l (add_max l' k acc) <->
...
Definition is_max k' k l acc :=
  ...
  | Some (Some k'') => k' = Some (Z.max k k'')
  | _ => k' = Some k
  end.

Lemma MapsTo_add_max l l' k k' acc :
  LevelMap.MapsTo (elt:=option Z) l k' (add_max l' k acc) <->
...
Proof.
  ...
  - rewrite LevelMapFact.F.add_mapsto_iff. intuition auto; subst.
    destruct (eqb_spec l l'); subst. unfold is_max. now rewrite hl. congruence.
    destruct (eqb_spec l l'); subst. unfold is_max. now rewrite hl. congruence.
    destruct (eqb_spec l l'); subst. unfold is_max in H. rewrite hl in H; subst.
    left; intuition auto. reflexivity.
    right. intuition auto.
Qed.
...
Definition update_max acc l k0 :=
  option_map2 Z.max (level_value acc l) (Some (Z.of_nat k0)).

(* Lemma update_max_spec acc l k : update_max acc l k0 = *)

Definition add_max_list l acc :=
  List.fold_right (fun '(l, k0) acc =>
    LevelMap.add l (update_max acc l k0) acc) acc l.
...
(*
Lemma In_fold_add_max l k les a :
  let map := add_max_list les a in
  LevelMap.MapsTo (elt:=option Z) l k map <->
  ((exists kl, In (l, kl) les /\ Some (Z.of_nat kl) = k
    /\ maximal_pre l kl les /\ maximal_map l kl a) \/
   (LevelMap.MapsTo l k a /\ (match k with Some k' => maximal_pre l (Z.to_nat k') les | None => True end))).
Proof.
  cbn; intros.
  unfold add_max_list.
  induction les in a |- *.
  - cbn. admit.
  - cbn. destruct a0.
    rewrite LevelMapFact.F.add_mapsto_iff. intuition auto.
    { red in H; subst k0. }
...
Lemma In_fold_add_max l k les a :
  ...
  ((exists kl, In (l, kl) les /\ Some (Z.of_nat kl) = k
   ...
   (LevelMap.MapsTo l k a /\ (match k with Some k' => maximal_pre l (Z.to_nat k') les | None => True end))).
Proof.
  cbn. unfold add_max_list.
  induction les in a |- *; cbn. admit.
  cbn. destruct a0.
  rewrite MapsTo_add_max.
  destruct (eqb_spec l k0). subst.


*)

Lemma In_fold_add_max k n a :
  LevelMap.In (elt:=option Z) k
  (LevelExprSet.fold
      (fun '(l, k0) acc => add_max l (Z.of_nat k0) acc) n a) <->
...
Lemma MapsTo_fold_add_max l n a :
  let map := LevelExprSet.fold (fun '(l, k0) acc => add_max l (Z.of_nat k0) acc) n a in
  (forall k, LevelMap.MapsTo (elt:=option Z) l k map ->
  ((exists kl, LevelExprSet.In (l, kl) n /\ Some (Z.of_nat kl) = k /\
    (forall kl', LevelExprSet.In (l, kl') n -> kl' <= kl) /\
    (forall kl', LevelMap.MapsTo l kl' a -> kl' ≤ Some (Z.of_nat kl))) \/
    (LevelMap.MapsTo l k a /\ (forall kl', LevelExprSet.In (l, kl') n -> Some (Z.of_nat kl') ≤ k))))
    ...
    * unfold is_max in H3 at 1.
      ...
      { destruct o.
        - subst k. pose proof (LevelMap.find_2 hfind). destruct H2 as [H2 Hnotin]. destruct (H2 _ H3).
          left. destruct H4 as [kl [hkl hleq]]. destruct hleq as [hleq hmax].
          noconf hleq. destruct (Z.max_spec (Z.of_nat k') (Z.of_nat kl)) as [[]|[]].
          { exists kl. split. apply H1. now right. split. f_equal. lia. split. intros.
            apply H1 in H6 as []. noconf H6. lia. now apply hmax. destruct hmax as [_ hmax].
          ...
          { exists k'. split. apply H1. now left. split. f_equal; lia. destruct hmax as [hmax hmax']; split.
            ...
            intros. transitivity (Some (Z.of_nat kl)). now apply hmax'. constructor; lia. }
          destruct (H2 _ H3) as [[kl [hkl hleq]]|]. noconf hleq.
          destruct hleq as [hleq hmax]. noconf hleq.
          destruct (Z.max_spec (Z.of_nat k') (Z.of_nat kl)) as [[]|[]].
          ...
          { left. exists k'. split. apply H1. now left. destruct hmax as [hmax hmax']. split. f_equal. lia. split.
            ...
            intros. transitivity (Some (Z.of_nat kl)) => //. 2:constructor; lia. now eapply hmax'. }
          destruct H4. clear H5.
          destruct (Z.max_spec (Z.of_nat k') z) as [[]|[]].
          { right. split. now rewrite H7.
            intros kl' hin. apply H1 in hin as [hin|hin]; noconf hin. constructor; lia.
            specialize (H6 _ hin). depelim H6; constructor; lia. }
          { left. exists k'. split. apply H1. now left. split. f_equal. lia. split.
            intros kl' hin. apply H1 in hin as []. noconf H8. lia.
            specialize (H6 _ H8). depelim H6. lia.
            intros. transitivity (Some z). 2:constructor; lia. eapply (LevelMapFact.F.MapsTo_fun H4) in H8. subst kl'. reflexivity. }
        - eapply LevelMap.find_2 in hfind. destruct H2 as [H2 hnotin]. specialize (H2 _ hfind) as [].
          subst k. destruct H2 as [kl [? []]]. congruence.
          destruct H2. subst k.
          left. exists k'. split; eauto. firstorder. split; eauto. split.
          { intros. eapply H1 in H3 as [hin|hin]; noconf hin. lia.
            specialize (H4 _ hin). depelim H4. }
          { intros kl' hin. apply (LevelMapFact.F.MapsTo_fun H2) in hin. subst; constructor. } }
...
Definition min_model_map (m : LevelMap.t (option Z)) cls : LevelMap.t (option Z) :=
  Clauses.fold (fun '(cl, concl) acc =>
    LevelExprSet.fold (fun '(l, k) acc =>
      add_max l (Z.of_nat k) acc) cl (add_max (levelexpr_level concl) 0%Z acc)) cls m.
...
Lemma min_model_mapsto_gen m cls :
  ...
  exists k, LevelMap.MapsTo l (Some k) (min_model_map m cls) /\
  (exists cl, Clauses.In cl cls /\
   exists n, k = Z.of_nat n /\ LevelExprSet.In (l,n) (premise cl) /\
   (* (forall cl', Clauses.In cl cls -> forall l k, LevelExprSet.In (l, k) (premise cl') -> k <= n) *)
   maximal_prem l n cls) \/ LevelMap.MapsTo l (Some k) m.
Proof.
  rewrite /min_model_map.
  eapply ClausesProp.fold_rec.
...
Lemma min_model_mapsto cls :
  forall l, LevelSet.In l (clauses_levels cls) ->
  exists k, LevelMap.MapsTo l (Some k) (min_model_map (LevelMap.empty _) cls) /\
  (exists cl, Clauses.In cl cls /\
   exists n, k = Z.of_nat n /\ LevelExprSet.In (l,n) (premise cl) /\
   (* (forall cl', Clauses.In cl cls -> forall l k, LevelExprSet.In (l, k) (premise cl') -> k <= n) *)
...
Equations? infer (cls : clauses) : infer_result (clauses_levels cls) cls :=
  infer cls := loop (clauses_levels cls) LevelSet.empty cls (init_model cls) (And4 _ _ _ _).
Proof.
  - now eapply clauses_conclusions_levels.
  - lsets.
  - red. now eapply init_model_levels.
  - red. intros k. now rewrite LevelSetFact.empty_iff.
Qed.
...
Proof.
  ...
  - exact H1.
  - intros x. now rewrite LevelSetFact.empty_iff.
Qed.
...
Definition premises_model_map (m : LevelMap.t (option Z)) cls : LevelMap.t (option Z) :=
  Clauses.fold (fun '(cl, concl) acc =>
    LevelExprSet.fold (fun '(l, k) acc =>
      add_max l (Z.of_nat k) acc) cl acc) cls m.
...
Definition undefined_model levels : LevelMap.t (option Z) :=
  LevelSet.fold (fun elt acc => LevelMap.add elt None acc) levels (LevelMap.empty _).

Definition premises_model V cl : LevelSet.t * model :=
  let levels := LevelSet.union (clause_levels cl) V in
  (levels, premises_model_map (undefined_model levels) (Clauses.singleton cl)).
...
Program Definition loop_check {V init cls} (m : valid_model V V init cls) (cl : clause) : result (premises_model V cl).1 LevelSet.empty cls (premises_model V cl).2 :=
  loop (premises_model V cl).1 LevelSet.empty cls (premises_model V cl).2 _.
Next Obligation.
  destruct m. split => //. 1-2:lsets. admit.
  - now intros x; rewrite LevelSetFact.empty_iff.
Admitted.

Equations check {V init cls} (m : valid_model V V init cls) (cl : clause) : bool :=
  check m cl with loop_check m cl :=
    | Loop => false (* Actually impossible *)
    | Model W v _ =>
      let '(concl, k) := concl cl in
      match LevelMap.find concl v.(model_model) with
      | Some None => false
      | Some (Some v) => (Z.of_nat k <=? v)%Z
      | None => false
      end.
...
infer_model cls with loop (clauses_levels cls) LevelSet.empty cls (init_model cls) _ :=
  | Loop => None
  | Model w vm heq => Some vm.(model_model).
Proof.
  split.
  - apply clauses_conclusions_levels.
  - lsets.
  - apply infer_obligation_3.
  - apply infer_obligation_4.
Qed.
...
Proof.
  ...
  destruct min_premise eqn:hmin => //= _.
  destruct cl as [prems [concl k]]; cbn.
  unfold level_value_above.
  destruct level_value eqn:hl => //.
  move/Z.leb_le.
  intros hrel.
  unfold interp_level. unfold level_value in hl. destruct LevelMap.find eqn:hfind => //. subst o.
  eapply LevelMap.find_2 in hfind.
  have conclm := valuation_of_model_spec _ _ _ hfind.
  set (v := (model_max _ - _ - _)%Z) in *.
  cbn in conclm. destruct conclm as [vpos conclm].
  ...
  eapply LevelMap.find_1 in vpmap. rewrite vpmap. lia.
Qed.
...
Proof.
  ...
  intros [= <-]. clear Heq Heqcall. destruct vm as [model ofV ovW clsconcl ism mext], heq; cbn in *.
  set (V := clauses_levels cls) in *.
  assert (total_model_of V model).
  { intros l inl. apply model_ext_le in mext. red in mext.
    ...
    depelim hmodel. now exists y. }
  unfold correct_model.
  have encl : enabled_clauses model cls.
  { eapply enabled_clauses_ext. apply mext.
    unfold enabled_clauses. eapply Clauses.for_all_spec. tc.
    ...
    eapply LevelMap.find_1 in hm. unfold level_value. now rewrite hm. }
  split => //.
  unfold clauses_sem.
  intros cl hin.
  eapply valid_clause_model. eapply Clauses.for_all_spec in encl; tc. now eapply encl in hin.
...
Proof.
  ...
  *)
  exfalso.
  have me := model_extends v.
  have me' := model_ext_le _ _ _ me.
  ...
  assert (min_premise undef_model prems = Some z).
  {
  (min_premise_pres _ _ _ )

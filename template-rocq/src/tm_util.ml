...
module RetypeMindEntry =
  ...
  let sup_sort s1 s2 =
    let open Sorts in
    match s1, s2 with
  | (_, SProp) | GQSort _, _ | _, GQSort _ -> assert false (* template SProp not allowed *)
  | (SProp, s) | (Prop, s) | (s, Prop) -> s
  | (Set, Set) -> Sorts.set
  | (Set, (Type u | VQSort (_, u))) | ((Type u | VQSort (_, u)), Set) -> Sorts.sort_of_univ (Universe.sup u Universe.type0)
  | ((Type u | VQSort (_, u)), (Type v | VQSort (_, v))) -> Sorts.sort_of_univ (Universe.sup u v)
